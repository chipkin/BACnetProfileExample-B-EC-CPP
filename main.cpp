...
static uint32_t g_deviceInstance = 389013;
...
static const char* DEVICE_DESCRIPTION =
    "Chipkin CAS BACnet Stack example - B-EC (Elevator Controller) profile. "
    "DS-RP/RPM/WP/WPM-B, DS-COV-B, DS-COVM-B, intrinsic alarming "
    "(AE-N-I-B / AE-ACK-B / AE-INFO-B), DM-DDB-A, DM-DCC-B, DM-TS-B, DM-RD-B. "
    "Commands three outputs and the Elevator Group's landing calls.";
...
// prints (it used to be a separate hardcoded "1.0.0" constant nobody
// updated as APP_VERSION advanced).
//
...
// banner's "CAS BACnet Stack version: X.Y.Z.W" line), so it can never go
// stale either - see g_firmwareRevision below, populated once right after
// LoadBACnetFunctions() succeeds.
static const char* VENDOR_NAME = "Chipkin Automation Systems";
static const char* MODEL_NAME = "CAS BACnet Stack Example - B-EC";
static std::string g_firmwareRevision;

// DeviceCommunicationControl / ReinitializeDevice password. A management station
// may include a password with either request; the device accepts the command
// only if it matches. It crosses the wire in PLAINTEXT - this is a guard against
// accidents, not a security boundary. Set your device's real secret before
// shipping, or leave empty to accept any request (no password required).
